(() => {
  ...
  async function meleeHit() {
    ...
    pushLog("SYS", "combat_attack(melee) 요청", "sys");
    const r = await postForm("./api/combat_attack.php", { user_key: uk, kind: "melee" });

    if (!r.ok) {
      pushLog("ERR", `combat_attack(melee) 실패: ${r.error || "unknown"}`, "err");
      if (r._meta) pushLog("ERR", `meta: ${JSON.stringify(r._meta)}`, "err");
      if (r.raw) pushLog("ERR", r.raw, "err");
      return;
    }
  ...
  async function gunFire() {
    ...
    pushLog("SYS", "combat_attack(gun) 요청", "sys");
    const r = await postForm("./api/combat_attack.php", { user_key: uk, kind: "gun" });

    if (!r.ok) {
      pushLog("ERR", `combat_attack(gun) 실패: ${r.error || "unknown"}`, "err");
      if (r._meta) pushLog("ERR", `meta: ${JSON.stringify(r._meta)}`, "err");
      if (r.raw) pushLog("ERR", r.raw, "err");
      return;
    }
  ...
  async function throwItem(item_id) {
    ...
    pushLog("SYS", `combat_attack(throw): ${item_id}`, "sys");

    const r = await postForm("./api/combat_attack.php", { user_key: uk, kind: "throw", item_id });
    if (!r.ok) {
      pushLog("ERR", `combat_attack(throw) 실패: ${r.error || "unknown"}`, "err");
      if (r._meta) pushLog("ERR", `meta: ${JSON.stringify(r._meta)}`, "err");
      if (r.raw) pushLog("ERR", r.raw, "err");
      openCombatModal();
      return;
