...
try {
  ...
  // loadout
  $st = $pdo->prepare("SELECT primary_item, secondary_item, melee_item, armor_item, armor_instance_id
  ...
  // raid
  $st = $pdo->prepare("SELECT status, inventory_json, throw_json, brought_json, armor_json
  ...
  echo j([
    ...
    'raid'=>[
      ...
      'armor'=>$armorJ, // { armor, durability, max, tier } 형태로 저장됨
    ]
  ]);
  exit;
